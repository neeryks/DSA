...
class MaxHeap:
    ...
    def bubble_up(self, index):
        ...
        if index == 1:
            return
        p_index = index//2
        if self.H[p_index] >= self.H[index]:
            return 
        self.H[p_index],self.H[index] = self.H[index],self.H[p_index]  
...
class MedianMaintainingHeap:
    ...
    def get_median(self):
        ...
        # your code here
        if self.hmin.size() - self.hmax.size() == 0:
            med = (self.hmin.min_element() + self.hmax.max_element())/2
            return med
        return self.hmin.min_element()
    ...
    def insert(self, elt):
        ...
        if elt < self.hmax.max_element():
            ...
            return
        elif elt >= self.hmax.max_element():
            self.hmin.insert(elt)
            self.balance_heap_sizes()
            return
